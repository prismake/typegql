...
In `decapi` to create schema, we need to pass class decorated with `@Schema` to `compileSchema` function
...
[Installation and setup](getting-started-setup)

**Important!** setup steps are simple, but required. Make sure to check [setup](getting-started-setup) section.
