...
```ts
import { ObjectType, Field, Arg, InputObjectType } from 'decapi'
...
class User {
  ...
  deleteManyTodos(where: WhereFilter): DeleteTodosResult {
    ...
    // BTW where argument is an instance of WhereFilter class here
    console.log(where instanceof WhereFilter) // true
