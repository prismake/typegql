...
There are 3 argument decorators in `decapi` that allow passing values to resolvers - `@Context`, `@Source`, `@Info`

Implementation of `Viewer` type would use `@Context` decorator to get user id from graphql context and could look like:

```ts
import { ObjectType, Field, Context } from 'decapi'
...
```ts
import { ObjectType, Field, Inject } from 'decapi';
